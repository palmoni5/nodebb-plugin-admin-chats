...
hooks.addProfileLink = async function (data) {
    ...
    // Nothing to moderate on your own profile — /user/<self>/chats is just the
    // regular chat list, already reachable from the main menu.
    if (parseInt(data.uid, 10) === parseInt(viewerUid, 10)) {
        return data;
    }
    ...
    data.links.push({
        ...
        visibility: {
            self: false, other: true, moderator: true, globalMod: true, admin: true,
        },
    });
