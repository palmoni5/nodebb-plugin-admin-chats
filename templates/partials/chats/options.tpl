<div class="d-flex gap-1 align-items-stretch">
	<!-- search -->
	<button class="btn btn-ghost btn-sm" component="chat/room/search/toggle" data-manual-tooltip="1" title="{{tx("global:header.search")}}">
		<i class="fa fa-search text-muted"></i>
	</button>
	<div component="chat/room/search/container" class="position-relative hidden align-self-center">
		<input component="chat/room/search" class="form-control form-control-sm" placeholder="{{tx("search:type-to-search")}}" style="width: 150px;">
	...
	<div class="dropdown d-flex" data-manual-tooltip="1" title="{{tx("modules:chat.notification-settings")}}" component="chat/notification/setting">
	...
	<!-- pinned messages -->
	<button component="chat/pinned/messages/btn" class="btn btn-ghost btn-sm d-none d-lg-block flex-nowrap" title="{{tx("modules:chat.pinned-messages")}}" data-bs-toggle="tooltip" data-bs-placement="bottom">
		<i class="fa fa-thumb-tack text-muted"></i>
	</button>

	<!-- manage/options dropdown -->
	<div class="dropdown d-flex" data-manual-tooltip="1" title="{{tx("modules:chat.options")}}">
		...
		<ul class="dropdown-menu dropdown-menu-end p-1 text-sm" component="chat/controls" role="menu">
			...
			<li role="presentation">
				<a href="#" role="menuitem" class="dropdown-item rounded-1 d-flex align-items-center gap-2 admin-chat-lock-toggle-item" data-room-id="{roomId}" data-locked="{{{ if adminChatLock.isLocked }}}true{{{ else }}}false{{{ end }}}">
					<i class="fa fa-fw {{{ if adminChatLock.isLocked }}}fa-lock-open{{{ else }}}fa-lock{{{ end }}}"></i>
					<span>{{{ if adminChatLock.isLocked }}}{{tx("admin-chats:menu.release")}}{{{ else }}}{{tx("admin-chats:menu.lock")}}{{{ end }}}</span>
				</a>
			</li>
			...
			<li>
				<a class="dropdown-item rounded-1 d-flex align-items-center gap-2" href="#" data-action="manage" role="menuitem">
					<i class="fa fa-fw text-muted fa-cog"></i> {{tx("modules:chat.manage-room")}}
				</a>
			</li>
			{{{ if isOwner }}}
			<li>
				<a class="dropdown-item rounded-1 d-flex align-items-center gap-2" href="#" data-action="rename" role="menuitem">
					<i class="fa fa-fw text-muted fa-edit"></i> {{tx("modules:chat.rename-room")}}
				</a>
			</li>
			{{{ end }}}
			{{{ if !adminAllChats }}}
			<li>
				<a class="dropdown-item rounded-1 d-flex align-items-center gap-2" href="#" data-action="leave" role="menuitem">
					<i class="fa fa-fw text-muted fa-sign-out"></i> {{tx("modules:chat.leave-room")}}
				</a>
			</li>
			{{{ end }}}
			{{{ if (public && isAdmin) }}}
	...
	<!-- users toggle -->
	{{{ if users.length }}}
	<div component="chat/user/list/btn" class="btn btn-ghost btn-sm d-none d-lg-flex flex-nowrap align-items-center gap-2" title="{{tx("modules:chat.view-users-list")}}" data-bs-toggle="tooltip" data-bs-placement="bottom">
